/**
 * Seed logic for the Apr 30, 2026 book (PRD §4) — shared by the CLI seed
 * (prisma/seed.ts) and the one-time /api/setup/seed bootstrap route.
 * Regression anchors documented inline below.
 */
import type { PrismaClient } from "@/lib/generated/prisma/client";

type SeedClient = Pick<
  PrismaClient,
  | "alert" | "recommendation" | "exposureSnapshot" | "marginRecord" | "marketQuote"
  | "hedge" | "cashBalance" | "position" | "hedgePolicy" | "counterparty" | "currency" | "user"
>;

export const AS_OF = new Date("2026-04-30T22:00:00.000Z");
const D = (s: string) => new Date(s + "T00:00:00.000Z");

export async function seedBook(prisma: SeedClient) {
